...
class MultiTaskRecognizer:
    ...
    def __init__(self, tasks):
        ...
        self.__position = {'gender': 0, 'smile': 1, 'glasses': 2, 'head_pose': 3}
        ...
        with self.__graph.as_default():
            ...
            #self.__labels_landmarks = tf.placeholder(tf.float32, shape=[None])
            ...
            output_gender, output_smile, output_glasses, output_head_pose = TCDCN(self.__images, self.__tasks)

            '''with tf.name_scope('loss_landmarks'):
                self.__losses_landmarks = tf.abs(tf.subtract(output_landmarks, self.__labels_landmarks))
                self.__loss_landmarks = tf.reduce_mean(self.__losses_landmarks)'''
            ...
            with tf.name_scope('optimizer'):
                ...
                self.__loss = self.__loss_gender + self.__loss_smile + self.__loss_glasses + self.__loss_head_pose
            ...
            with tf.name_scope('predictions'):
                #self.__labels_predicted_landmarks = output_landmarks
                if 'gender' in self.__tasks:
        ...
        for epoch in range(epochs):
            ...
            for step in tqdm.tqdm(range(num_steps), desc='Epoch ' + str(epoch + 1 + self.__GLOBAL_EPOCH) + '/' + str(epochs + self.__GLOBAL_EPOCH)):
                ...
                for _ in range(self.__BATCH_SIZE):
                    ...
                    row = row.strip().split()
                    directory = row[0]
                    img = dp.load_image(self.__DATA_DIR + directory, self.__shape)
                    land = [float(number) for number in row[1: 11]]
                    gend = int(row[11]) - 1
                    sm = int(row[12]) - 1
                    glass = int(row[13]) - 1
                    hp = int(row[14]) - 1
                ...
                _, l = self.__session.run([self.__train_op, self.__loss],
                                          feed_dict=self.__create_feed_dict__(images, [gender, smile, glasses, head_pose]),
                ...
                if step > 0 and step % 3 == 0:
                    ...
                    for _ in range(test_steps):
                        ...
                        pred = self.__session.run([self.__out[k] for k in self.__tasks if k],
                                                    feed_dict=self.__create_feed_dict__([img], [[gend], [sm], [glass], [hp]]),
                                                    run_metadata=run_metadata)
...
a = MultiTaskRecognizer(tasks=['glasses', 'smile', 'head_pose', 'gender'])
a.train()
