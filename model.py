    ...
    def train(self, epochs=1):
        ...
        for epoch in range(epochs):
            ...
            for step in tqdm.tqdm(range(num_steps), desc='Epoch ' + str(epoch + 1 + self.__GLOBAL_EPOCH) + '/' + str(epochs + self.__GLOBAL_EPOCH)):
                ...
                to_send_over = [self.__train_op, self.__total_loss] + [self.__loss[task] for task in self.__tasks]
                l = self.__session.run(to_send_over ,
                ...
                if (step % 100 == 0 and step > 0) or step == num_steps - 1:
                    ...
                    for task in self.__tasks +['total_loss']:
                        with open(self.__TMP_DIR + '/log_loss_' + task + '.txt', 'a', encoding='utf8') as f:
                            f.write(str(loss[task] / step) + '\n')
                    
                    print("Loss: ", str(loss['total_loss'] / step))    
